...
export default function ListPage() {
  ...
  const filteredFractions = mockFractions.filter(fraction =>
            ...
            <div className="max-w-6xl mx-auto">
              <div className="grid gap-4">
                {filteredFractions.map((fraction) => (
                  <div
                    key={fraction.id}
                    className="bg-[#0B78FD1A] border border-white/[0.04] rounded-lg p-6 hover:border-white/[0.08] transition-all duration-200"
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-4">
                        ...
                        <span
                          className={`px-3 py-1 rounded-full text-sm font-polysans ${getStatusColor(
                            fraction.status
                          )}`}
                        >
                          {fraction.status}
                      ...
                      <div className="text-gray-400 text-sm font-polysans">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        <div className="text-gray-400 font-polysans mb-1">Creator</div>
                        <div className="text-white font-polysans font-mono">
                          {fraction.creator}
                        </div>
                      </div>
                      <div>
                        <div className="text-gray-400 font-polysans mb-1">Recipients</div>
                        <div className="text-white font-polysans">
                          {fraction.recipients} addresses
                        </div>
                      </div>
                      <div>
                        <div className="text-gray-400 font-polysans mb-1">Total Split</div>
                        <div className="text-white font-polysans font-medium">
                          {fraction.totalSplit}
                        </div>
                      </div>
                      <div className="flex justify-end">
                        <button className="px-4 py-2 rounded-lg bg-[#4E88F0] text-white font-polysans font-medium hover:bg-[#4E88F0]/90 transition-all duration-200">
                          View Details
                        </button>
                      </div>
                    </div>
